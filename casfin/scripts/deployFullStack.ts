...
async function main() {
  ...
  const casinoConfigTxs: Record<string, unknown> = {};

  for (const game of deployedGameContracts) {
    casinoConfigTxs[game.authorizeLabel] = await trackTransaction(
      game.authorizeLabel,
  ...
  casinoConfigTxs.setCoinFlipResolver = await trackTransaction(
    "setCoinFlipResolver",
    coinFlip.contract.setResolver(deployer.address, true)
  );
  casinoConfigTxs.setDiceResolver = await trackTransaction(
    "setDiceResolver",
    dice.contract.setResolver(deployer.address, true)
  );
  casinoConfigTxs.setCrashResolver = await trackTransaction(
    "setCrashResolver",
